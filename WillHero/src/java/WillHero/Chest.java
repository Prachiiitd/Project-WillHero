...
public abstract class Chest  {
    ...
    public ImageView setOpenChest(int x, int y) throws NullPointerException {
        try {
    ...
    public void collide(Object object) {
        if (object instanceof Hero) {
            ...
            if (hero.getHero().getBoundsInParent().intersects(chest.getBoundsInParent())) {
                if(chest.getImage().equals(new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("chestClosed.png"))))) {
                    chest.setImage(new Image(Objects.requireNonNull(this.getClass().getResourceAsStream("chestOpen.png"))));
                    getContent(hero);
                }
            }
        }
