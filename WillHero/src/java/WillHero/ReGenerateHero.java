    ...
    public Hero getHero(String fileName) throws IOException, ClassNotFoundException {
        ...
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fileName))) {
            try {
                ...
                Weapon weapon2 = new Weapon2(hero.getHelmet().getWeapon(1).getDamage(), hero.getHelmet().getWeapon(1).getRange(), hero.getHelmet().getWeapon(1).isActive(), hero.getHelmet().getWeapon(1).isVisible(), hero.getHelmet().getWeapon(1).getX(), hero.getHelmet().getWeapon(1).getY());
